...
//        return json($data);
    ...
    public function addWaring(Request $request)
    {
        // $id = $request->param('id');
        return $this->fetch();
    }

    public function saveWaring(Request $request)
    {
        if (Request::instance()->isAjax()) {
            $data = $request->param();
            $data['time'] = date('Y-m-d H:i:s', time());
            $res = Db::table('jh_waring')->insert($data);
            if ($res == 1) {
                $result = [
                    'status' => 'success',
                    'msg' => '保存成功！',
    ...
    public function delete(Request $request)
    {
        if (Request::instance()->isAjax()) {
            $id = $request->param('id');
            $res = Db::table('jh_waring')->where('id', $id)->delete();
            if ($res > 0) {
                $result = [
                    'status' => 'success',
                    'msg' => '删除成功！',
